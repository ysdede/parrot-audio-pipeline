...
# Set a threshold for common and rare words
common_threshold = 10  # Adjust as needed
rare_threshold = 3     # Adjust as needed
...
for rare_word in rare_words:
    for common_word in common_words:
        if similarity(rare_word, common_word) > 0.8:  # Threshold for similarity, adjust as needed
...
formatted_typos = ['{} - {}'.format(typo[0], typo[1]) for typo in potential_typos]

# Write the formatted strings to a file
with open('output_file.txt', 'w') as file:
    file.write('\n'.join(formatted_typos))

# Save unique words with their frequencies to a file
with open('unique_words_with_freq.txt', 'w', encoding='utf-8') as file:
